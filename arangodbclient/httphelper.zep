...
class HttpHelper
{
    ...
    public static function createConnection(<ConnectionOptions> options)
    {
        ...
        let endpoint = options[ConnectionOptions::OPTION_ENDPOINT];
        ...
        let fp =  stream_socket_client(endpoint, errNo, message, options[ConnectionOptions::OPTION_TIMEOUT], STREAM_CLIENT_CONNECT, context);
        if !fp {
            throw new ConnectException("cannot connect to endpoint '" . options[ConnectionOptions::OPTION_ENDPOINT] . "': " . message, errNo);
        }
